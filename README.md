...
3. Copy the `emoji-language-server` binary to your Zed extensions directory:
   ...
   cp target/release/emoji-language-server ~/.zed/extensions/emoji-completions
   ```
...
- Non-language specific support (I don't think Zed supporrts this yet?). Currently this only works in file types where the language server is explicitly activated.
